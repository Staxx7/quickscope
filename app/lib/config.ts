...
export const config = {
  ...
  quickbooks: {
    ...
    redirectUri: process.env.QB_REDIRECT_URI || `${process.env.NEXT_PUBLIC_APP_URL}/api/auth/quickbooks/callback`,
    environment: process.env.QB_ENVIRONMENT || 'sandbox',
    discoveryDocumentUrl: process.env.QB_DISCOVERY_DOCUMENT_URL || 'https://developer.intuit.com/.well-known/openid_configuration',
    tokenUrl: process.env.QB_TOKEN_URL || 'https://oauth.platform.intuit.com/oauth2/v1/tokens/bearer',
    authorizationUrl: process.env.QB_AUTHORIZATION_URL || 'https://appcenter.intuit.com/connect/oauth2',
    scope: process.env.QB_SCOPE || 'com.intuit.quickbooks.accounting',
    webhookVerifierToken: process.env.QB_WEBHOOK_VERIFIER_TOKEN || '',
...
export function validateConfig(): { isValid: boolean; missingVars: string[] } {
  ...
  if (!config.quickbooks.clientId) missingVars.push('QB_CLIENT_ID');
  if (!config.quickbooks.clientSecret) missingVars.push('QB_CLIENT_SECRET');
